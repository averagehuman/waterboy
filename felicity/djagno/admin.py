...
from felicity import config, settings
...
class FelicityForm(forms.Form):
    version = forms.CharField(widget=forms.HiddenInput)

    def __init__(self, initial, *args, **kwargs):
        super(FelicityForm, self).__init__(*args, initial=initial, **kwargs)
        version_hash = hashlib.md5()

        for name, (default, help_text) in settings.FELICITY_CONFIG.items():
            config_type = type(default)
            if config_type not in FIELDS:
                raise ImproperlyConfigured(_("Felicity doesn't support "
    ...
    def save(self):
        for name in settings.FELICITY_CONFIG:
...
class FelicityAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        initial = dict(default_initial,
            **dict(config.backend.mget(settings.FELICITY_CONFIG.keys())))
        form = FelicityForm(initial=initial)
        if request.method == 'POST':
            form = FelicityForm(data=request.POST, initial=initial)
            if form.is_valid():
                form.save()
        ...
        return render_to_response('admin/felicity/change_list.html',
...
admin.site.register([Config], FelicityAdmin)
